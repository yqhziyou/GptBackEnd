...
const generateAIResponse = async (userId, sessionId, userMessage, aiModel) => {
    try {
        ...
        let session = await Session.findOne({ sessionId, userId }); // Ensure the session belongs to the user
        let messages = [];
        ...
        if (session) {
            ...
            const response = await openai.chat.completions.create({
                model: aiModel,  // Model to use
                messages: [{ role: "system", content: "Session initialization." }], // Initial message to ensure model loading
                stream: false
            });
            ...
            await addSessionToUser(userId, session._id); // Pass session's ObjectId
        }
